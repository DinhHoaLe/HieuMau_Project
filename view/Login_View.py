...
class LoginView:
    def __init__(self, root):
        ...
        self.label_header = tk.Label(self.frame, text="ĐĂNG NHẬP", font=self.header_font, fg="#8B0000",
                                     bg=background_color)
        ...
        self.label_username = tk.Label(self.frame, text="Tên người dùng:", font=self.label_font, anchor='w',
                                       fg=label_color, bg=background_color)
        self.label_username.grid(row=2, column=0, padx=40, pady=10, sticky='w')

        self.entry_username = tk.Entry(self.frame, width=30, font=self.input_font, bg=entry_background_color, bd=2,
                                       relief="solid", )
        self.entry_username.grid(row=2, column=1, padx=40, pady=10, sticky='e')

        # Tạo label và entry cho mật khẩu
        self.label_password = tk.Label(self.frame, text="Mật khẩu:", font=self.label_font, anchor='w', fg=label_color,
                                       bg=background_color)
        self.label_password.grid(row=3, column=0, padx=40, pady=10, sticky='w')

        self.entry_password = tk.Entry(self.frame, show="*", width=30, font=self.input_font, bg=entry_background_color,
                                       bd=2, relief="solid")
        ...
        self.checkbox_show_password = tk.Checkbutton(self.frame, text="Hiển thị mật khẩu",
                                                     variable=self.show_password_var, bg=background_color,
                                                     command=self.toggle_password_visibility)
        self.checkbox_show_password.grid(row=4, column=1, padx=40, pady=10, sticky='e')

        # Tạo nút đăng nhập
        self.button_login = tk.Button(self.frame, text="Đăng nhập", command=self.handle_login, width=20,
                                      font=self.button_font, bg=button_color, fg="white")
